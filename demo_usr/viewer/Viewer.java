...
boolean connect() throws UnknownHostException {
    try {
        ...
        return true;
    } catch (UnknownHostException uke) { throw uke;
...
protected void collectData(){
    ...
    while (true) {
        ...
        try {
            ...
            saveGraphData(destDir, "dot", Integer.toString(
                    count), graph);
            ...
            do {
                ...
                if (processData == null) {
                    System.err.println(
                        "Viewer: pipe error - restarting pipe");
                    process.stop();
                }
            ...
            saveGraphData(destDir, "xdot", Integer.toString(
                    count), xdot);
 ...
 */
protected boolean saveGraphData(File destDir,
    String extention,
    String count,
    String data)                                                {
    try {
        File file = new File(destDir,
            "graph-" + count + "." + extention);
 ...
 */
protected String getGraphData(File destDir,
    String extention,
    String count)                                      {
    try {
        File file = new File(destDir,
...
protected PipeProcess startDot() throws IOException {
    // create a subrocess
    String [] processArgs =
    { "/usr/bin/env", "dot", "-Txdot", "-K" + layout };
 ...
 */
protected String sendDataToProcess(PipeProcess pipe,
    String data)                  {
    ...
    if (result == null) {
        ...
        System.err.println(
            "Viewer: dot converted " + data.length() + " to " +
            result.length());
