...
public abstract class AbstractRouterFabric implements RouterFabric, NetIFListener, DatagramDevice {
    ...
    void receiveRoutingTable(byte [] bytes, NetIF netIF)
    {
        ...
        if (t == null) {
            Logger.getLogger("log").logln(1<<6, leadin()+ " not merging null routing table received on "+netIF);
        } else {
            Logger.getLogger("log").logln(1<<6, leadin()+ " merging routing table received on "+netIF);
        }
        ...
        synchronized (table_) {
            ...
            Logger.getLogger("log").logln(1<<6, leadin()+ " merged routing table received on "+netIF + "\nResult = " + table_);
