...
BlockingDeque<Waker> inWaitQueue_ = null;       // Queue stores objects
                                                // waiting for
...
throws NoRouteToHostException, usr.net.InterfaceBlockedException {
    ...
    if (inQueueDiscipline_ == QUEUE_NOQUEUE) {
        ...
        if (inIsBlocking() && waitObj !=
            null) throw new usr.net.InterfaceBlockedException();
        inDroppedPacket(dg);
        return false;
    } else {
...
/** Constructor sets up */
InQueueHandler(int discipline,
    BlockingQueue<DatagramHandle> q,
    FabricDevice f){
...
OutQueueHandler(int discipline,
    BlockingQueue<DatagramHandle> q,
    FabricDevice f){
    queueDiscipline_ = discipline;
    queue_ = q;
