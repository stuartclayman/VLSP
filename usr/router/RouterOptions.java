...
String APOutputPath_ = null;            // Path to which infosource and
                                        // aggpoint should write
...
SAXParseException, SAXException {
    ...
    if (!basenodeName.equals("RouterOptions")) throw new
              SAXException(
...
void processOutputParameters(NodeList out) throws SAXException {
    if (out.getLength() > 1) throw new SAXException(
...
void processRoutingParameters(NodeList rps) throws SAXException {
    if (rps.getLength() > 1) throw new SAXException(
    ...
    try {
        ...
        DatagramFactory.setClassForProtocol(dgtype,
            Protocol.CONTROL);
    } catch (ClassNotFoundException e) { throw new SAXException(
                                   ...
                                   " in Routing options" +
                                   e.getMessage());
    } catch (ClassCastException e) { throw new SAXException(
                                         "Class name " + dgtype +
...
void processAPM(NodeList apm) throws SAXException {
    if (apm.getLength() > 1) throw new SAXException(
...
private void processMonitoring(NodeList list) throws SAXException {
    if (list.getLength() > 1) throw new SAXException(
