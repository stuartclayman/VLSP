...
public synchronized boolean addNetIF(NetIF inter,
    ...
    Logger.getLogger("log").logln(
        USR.STDOUT, "SimpleRoutingTable: ADD LOCAL NET IF " +
        inter.getAddress());
    //Logger.getLogger("log").logln(USR.ERROR, "SimpleRoutingTable:
    ...
    if (table_.get(a) == null) {
        SimpleRoutingTableEntry e1 = new SimpleRoutingTableEntry(
            a, 0,
            null);
        table_.put(a, e1);
        changed1 = true;
 ...
 */
public synchronized boolean mergeTables(RoutingTable table,
    NetIF inter,
    RouterOptions options)                                            {
    ...
    Collection <SimpleRoutingTableEntry> es =
        (Collection <SimpleRoutingTableEntry>)table2.getEntries();
    ...
    if (inter != null) {
        for (SimpleRoutingTableEntry e : (Collection <
            ...
            if (inter.equals(e.getNetIF())) {
                SimpleRoutingTableEntry e2 =
                    (SimpleRoutingTableEntry)table2.getEntry(a);
                ...
                if (receivedCost > e.getCost()) {
                    ...
                    //Logger.getLogger("log").logln(USR.ERROR, "COST
                    // CHANGE");
                    changed = true;
                }
            }
 ...
 * a change
 */
public synchronized boolean  mergeEntry(
    SimpleRoutingTableEntry newEntry,
    NetIF inter,
    RouterOptions options)
{
    ...
    if (oldEntry == null) {
        ...
        if (newCost > options.getMaxDist()) {
            ...
            // Create new entry
            SimpleRoutingTableEntry e = new SimpleRoutingTableEntry(
                addr, newCost, inter);
            //Logger.getLogger("log").logln(USR.ERROR, "NEW ENTRY
            // ADDED");
            table_.put(addr, e);
    ...
    if (inter != null && inter.equals(oldEntry.getNetIF())) {
        if (newCost > oldEntry.getCost()) {
            if (newCost > options.getMaxDist())
                table_.remove(addr);                    // Too far, can
                                                        // no longer
                                                        // route
            else
                oldEntry.setCost(newCost);
...
public synchronized String toString(){
    ...
    Collection<SimpleRoutingTableEntry> rtes =
        (Collection <SimpleRoutingTableEntry>)getEntries();
...
public synchronized byte[] toBytes(){
    ...
    if (entrySize == 0) throw new Error(
            "SimpleRoutingTable: entrySize is 0");
