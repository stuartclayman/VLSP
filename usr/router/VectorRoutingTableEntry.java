...
VectorRoutingTableEntry(Address addr,
    List<Integer> costs,
    List<NetIF>   inters){
...
VectorRoutingTableEntry(Address addr, Integer[] costs, NetIF[] inters){
    if (costs.length != inters.length) throw new Error(
            "costs length != interfaces length");

    if (costs.length == 0) throw new Error("costs length == 0");
...
VectorRoutingTableEntry(int addressSize,
    byte [] tableEntry,
    NetIF inter) throws Exception {
