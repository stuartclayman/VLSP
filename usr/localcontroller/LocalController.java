...
private String routerConfig_ = "";      // String contains config for
                                        // routers
...
public static void main(String[] args){
    ...
    if (args.length != 1) {
        Logger.getLogger("log").logln(
            USR.ERROR, "Command line must specify " +
    ...
    try {
        ...
        Logger.getLogger("log").logln(
            USR.ERROR, "Unable to understand port number." +
            e.getMessage());
        System.exit(-1);
    }
...
public LocalController (int port){
    try {
        ...
        Logger.getLogger("log").logln(
            USR.ERROR, "Cannot find host info for controller");
        shutDown();
    }
...
public void shutDown(){
    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() +
        "Local controller got shutdown message from global controller.");

    //ThreadTools.findAllThreads("LC top of shutDown:");

    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() +
        "Stopping all running routers");
        ...
        for (int i = 0; i < routers_.size(); i++) {
            ...
            try {
                ...
                Logger.getLogger("log").logln(USR.ERROR,
                    leadin() +
                    "Cannot send shut down to Router");
                Logger.getLogger("log").logln(USR.ERROR,
                    e.getMessage());
            ...
            //ThreadTools.findAllThreads("LC after router
            // shutDown:");
        }
    }
    ...
    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() +
        "Stopping global controller interactor");
    try {
        gcInteractor_.quit();
    } catch (Exception e) {
        Logger.getLogger("log").logln(USR.ERROR,
            leadin() +
            "Cannot exit from global interactor");
...
public void aliveMessage(LocalHostInfo gc){
    ...
    Logger.getLogger("log").logln(
        USR.STDOUT, "Got alive message from global controller.");
    try {
        Logger.getLogger("log").logln(USR.STDOUT,
            "Sending to " + gc.getName() + ":" +
            gc.getPort());
        gcInteractor_ = new GlobalControllerInteractor(gc);
        gcInteractor_.respondToGlobalController(hostInfo_);
    } catch (UnknownHostException e) {
        Logger.getLogger("log").logln(
            USR.ERROR, "Cannot contact global controller");
        Logger.getLogger("log").logln(USR.ERROR, e.getMessage());
        System.exit(-1);
    } catch (IOException e) {
        Logger.getLogger("log").logln(
 ...
 */
public String requestNewRouter(int routerId,
    int port1,
    int port2,
    String address,
    String name)
{
    ...
    try {
        ...
        Logger.getLogger("log").logln(USR.ERROR,
            leadin() +
            "Cannot construct address for router");
        return null;
    }
    ...
    for (tries = 0; tries < MAX_TRIES; tries++) {
        ...
        // try and connect
        try {
            // connect to ManagementConsole of the router on port
            // port1
    ...
    if (!isOK) {
        ...
        Logger.getLogger("log").logln(USR.ERROR,
            leadin() +
            "Unable to connect to Router on port "
            + port1);
        // stop process
        if (pw != null)
            pw.stop();
        ...
        try {
            ...
            Logger.getLogger("log").logln(USR.ERROR,
                leadin() +
                "IOException connecting to Router on port "
                + port1);
            Logger.getLogger("log").logln(USR.ERROR,
                leadin() + e.getMessage());
    ...
    BasicRouterInfo br = new BasicRouterInfo(routerId, 0, hostInfo_,
        port1);
    routers_.add(br);
    ...
    try {
        ...
        if (latticeMonitoring)
            interactor.monitoringStart(monitoringAddress,
                monitoringTimeout);
    } catch (IOException ioexc) {
        Logger.getLogger("log").logln(USR.ERROR,
            leadin() +
            "IOException setting interactor details for Router on port "
            + port1);
        pw.stop();
        child = null;
        try {
        ...
        Logger.getLogger("log").logln(USR.ERROR,
            leadin() +
            "MCRP Exception setting interactor details for Router on port "
            + port1);
        pw.stop();
        child = null;
        try {
 ...
 */
public String connectRouters(LocalHostInfo r1,
    LocalHostInfo r2,
    int weight,
    String name)
{
    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() +
        "Got connect request for routers");
    ...
    if (ri == null) {
        ...
        try {
            ...
            if (name == null) {
                connectionName = ri.createConnection(address,
                    weight);
            } else {
                connectionName =
                    ri.createConnection(address, weight,
                        name);
            }
...
public boolean sendRouterStats(List<String> list){
    synchronized (gcInteractor_) {      // Synchronize on gcInteractor
                                        // to
                                        // prevent overlap
        ...
        try {
            ...
            Logger.getLogger("log").logln(USR.ERROR,
                leadin() +
                "Cannot send stats to global controller "
                + e.getMessage());
            return false;
        }
        return true;
...
public boolean endRouter(LocalHostInfo r1){
    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() +
        "Got terminate request for router " +
        r1);
    RouterInteractor ri = findRouterInteractor(r1.getPort());
    if (ri == null) {
        Logger.getLogger("log").logln(USR.STDOUT,
            leadin() +
            "Cannot find router interactor -- router assumed closed");
        return true;
    }
    ...
    try {
        Logger.getLogger("log").logln(USR.STDOUT,
            leadin() +
            "Sending terminate request via interactor");
        ri.shutDown();
    }catch (Exception e) {
        Logger.getLogger("log").logln(USR.ERROR,
            leadin() +
            "Error shutting down router");
        Logger.getLogger("log").logln(USR.ERROR,
            leadin() + e.getMessage());
        return false;
    }
    ...
    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() +
        "Got shutdown response from router " +
        r1);
    ...
    synchronized (routerInteractors_) {
        ...
        if (i == routers_.size()) {
            Logger.getLogger("log").logln(USR.ERROR,
                leadin() +
                "Router not registered with localcontroller");
            return false;
        }
...
/** Local controller receives request to end a router */
public boolean endLink(LocalHostInfo r1,
    String r2Addr)                    {
    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() +
        "Got terminate request for link from"
        + r1 + " to " + r2Addr);
    RouterInteractor ri = findRouterInteractor(r1.getPort());
    if (ri == null)
        return false;
...
public boolean setAP(int GID, int AP){
    ...
    try {
        ...
        Logger.getLogger("log").logln(USR.ERROR,
            leadin() +
            "cannot set aggregation point for router "
            + GID);
        return false;
    }
    return true;
 ...
 */
public void startMonitoring(InetSocketAddress socketAddress,
    int timeout)                                  {
    ...
    if (latticeMonitoring) {
        for (int i = 0; i < routers_.size(); i++) {
            ...
            try {
                ...
                Logger.getLogger("log").logln(USR.ERROR,
                    leadin() +
                    "Cannot send stopMonitoring to Router");
                Logger.getLogger("log").logln(USR.ERROR,
                    e.getMessage());
            } catch (JSONException e) {
                Logger.getLogger("log").logln(USR.ERROR,
 ...
 */
public JSONObject onRouter(int routerID, String className,
    String[] args)                               {
    BasicRouterInfo br = routerMap_.get(routerID);
...
private RouterInteractor findRouterInteractor(int port){
    ...
    Logger.getLogger("log").logln(USR.ERROR,
        leadin() +
        "Unable to find router interactor listening on port "
        + port);
    return null;
}
