...
public void setAP(int gid, int ap, int cost,
    GlobalController g)                          {
...
public void controllerUpdate(long time,
    GlobalController g)                        {
...
public void addAccessPoint(long time, int gid,
    GlobalController g)                   {
    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() + "Node " + gid + " becomes AP");
    if (APGIDs_.indexOf(gid) != -1) {
        Logger.getLogger("log").logln(
            USR.ERROR,
            "AP controller found access point present when adding");
        return;
    }
    lse_.newAP(time, gid);
    APGIDs_.add(gid);
...
public void removeAccessPoint(long time, int gid){
    ...
    if ((index = APGIDs_.indexOf(gid)) == -1) {
        Logger.getLogger("log").logln(
            USR.ERROR,
            "AP controller could not find access point when removing");
        return;
    }
    Logger.getLogger("log").logln(USR.STDOUT,
        leadin() + " removing access point " +
        gid);
...
/** Return the gid and cost of the closest AP or null if there is no
 * such AP*/
public Pair<Integer, Integer> findClosestAP(int gid,
    GlobalController g)                        {
    ...
    while (tempLen > 0) {
        ...
        //System.err.println("Considering node "+cheapNode+" cost
        // "+cheapCost);
        if (isAP(cheapNode)) {         // Found cheapest AP]
            //System.err.println("Found "+cheapNode+" cost
            // "+cheapCost);
        ...
        int link;
        //System.err.println("Adding links from "+cheapNode+" cost
        // "+cheapCost);
        ...
        for (int i = 0; i < out.length; i++) {
            ...
            //System.err.println("Add "+link+" at pos "+tempLen+"
            // Link
            // cost ="+newCost+ " out cost was "+outCost[i]);
...
boolean removable(int gid, GlobalController g){
    ...
    while (toVisit.size() > 0) {
        ...
        for (int l : g.getOutLinks(newNode)) {
            if (toVisit.indexOf(l) == -1 && visited.indexOf(l) ==
                -1)
                toVisit.add(l);
        }
    }
...
public int getScore(long tim, int gid,
    GlobalController g)                  {
    return 0;
}
