 ...
 * nodes.
 */
public class ColouredNetworkAndApplicationVisualizationWithoutClustering implements Visualization {
	GlobalController gc;

	String localHost;

	public ColouredNetworkAndApplicationVisualizationWithoutClustering() {
		try {
			// get hostname only
	...
	public void visualize(PrintStream s) {
		...
		// visit each host & show localhost only
		for (String host : routerLocations.keySet()) {
			//if (localHost.startsWith(host)) {
