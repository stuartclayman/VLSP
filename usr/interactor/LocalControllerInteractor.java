...
public class LocalControllerInteractor {
     ...
     */
    public LocalControllerInteractor(String addr, int port) throws
     ...
     * @param port the port the server is listening on
     */
    public LocalControllerInteractor(InetAddress addr, int port) throws
    UnknownHostException,
    IOException {
     ...
     */
    public LocalControllerInteractor(LocalHostInfo lh) throws
    ...
    private synchronized void initialize(InetAddress addr, int port) {
        ...
        // null,
        // null);
        localControllerURI = "http://" + addr.getHostName() + ":"
            + Integer.toString(port);

        //Logger.getLogger("log").logln(USR.STDOUT, "localControllerURI:
        // " +
        // localControllerURI);
    ...
    JSONException {
        String uri = localControllerURI + "/command/"
            + java.net.URLEncoder.encode(str, "UTF-8");

        Logger.getLogger("log").logln(USR.STDOUT, "LC call: "
                                      + uri.substring(0, Math.min(84,
                                                                  uri.length())));

        JSONObject jsobj = rest.json(uri).toObject();

        Logger.getLogger("log").logln(USR.STDOUT,
                                      "LC response: " + jsobj.toString());
     ...
     */
    public JSONObject newRouter(int routerId, int port) throws IOException,
    JSONException {
     ...
     * Name is optional.
     */
    public String connectRouters(String host1, int port1, String host2, int port2, int weight, String name) throws IOException,
    JSONException {
        String toSend = MCRP.CONNECT_ROUTERS.CMD + " " + host1 + ":"
            + port1
            + " " + host2 + ":" + port2 + " " + weight;
    ...
    public Boolean endRouter(String host1, int port1) throws IOException,
    JSONException {
        String toSend = MCRP.ROUTER_SHUT_DOWN.CMD + " " + host1 + ":"
            + port1;
        JSONObject response = interact(toSend);
    ...
    public Boolean endLink(String host1, int port1, String address) throws IOException,
    JSONException {
        String toSend = MCRP.END_LINK.CMD + " " + host1 + ":" + port1
            + " "
            + address;
        JSONObject response = interact(toSend);
    ...
    JSONException {
        String toSend = MCRP.ROUTER_CONFIG.CMD + " "
     ...
     */
    public JSONObject appStart(int routerId, String className, String[] args) throws IOException,
    JSONException {
     ...
     */
    public Boolean checkLocalController(String host, int port) throws IOException,
    JSONException {
     ...
     * Check with a local controller.
     */
    public Boolean checkLocalController(LocalHostInfo gc) throws
    IOException, JSONException {
        String host = gc.getName();
        int port = gc.getPort();
        String toSend = MCRP.CHECK_LOCAL_CONTROLLER.CMD + " " + host
            + " " + port;
        JSONObject response = interact(toSend);
    ...
    /** Request stats from routers -- stats are returned by a separate
     * command*/
    public void requestRouterStats() throws IOException,
    JSONException {
     ...
     */
    public List<String> getRouterStats() throws IOException,
    JSONException {
        // 285-1 localnet  InBytes=0 InPackets=0 InErrors=0 InDropped=0
        // OutBytes=17890 OutPackets=500 OutErrors=0 OutDropped=0
        // InQueue=0
        // OutQueue=0
        // 285-1 /Router-15151-15152/Connection-1  InBytes=66
        // InPackets=1
     ...
     */
    public Boolean monitoringStart(InetSocketAddress addr, int howOften) throws
    IOException,
    JSONException {
