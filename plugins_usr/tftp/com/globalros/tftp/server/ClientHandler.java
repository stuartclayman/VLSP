...
import plugins_usr.tftp.com.globalros.tftp.common.TFTPOptions;
...
public class ClientHandler
{
     ...
     * @return
     */
    public void connect(Address address, int clientPort)
    {
        tftpSock.connect(address, clientPort);
    }
    ...
    //		catch (Exception e)
    //		{
    //			tftpLog.logln(USR.ERROR, getClient() + " UDP send packet failure.");
    ...
    //			{
    //            tftpLog.logln(USR.ERROR, "IOException: " + ios.getMessage());
    //				return null;
    //			}
    //
    ...
    public boolean receiveFileFromClient(OutputStream os, Address clientAddress, int clientPort, boolean useOptions)
    {
        ...
        if (useOptions)
        {
            ...
            tftpLog.logln(USR.ERROR, "ClientHandler: receiveFileFromClient transfer size = " + getTransferSize());
        ...
        do
        {
            ...
            {
                tftpLog.logln(USR.ERROR, "CODING ERROR? this case should be fetched in DataTransfer!");
                return false;
            }
            catch (InstantiationException e)
            {
                tftpLog.logln(USR.ERROR, "CODING ERROR? cannot construct DATA and ACK packets in receiveFileFromClient");
                return false;
            }
            catch (IOException e)
            {
                sendErrPacket(ERROR.ERR_NOT_DEFINED, e.getMessage());
                tftpLog.logln(USR.ERROR, getClient() + e.getMessage());
                return false;
            }
            catch (Throwable t)
            {
                t.printStackTrace();
                tftpLog.logln(USR.ERROR, "Some unrecoverable error occured. cannot continue with client: " + t);
                return false;
            }
            ...
            tftpLog.logln(USR.ERROR, "totalSize of receiveid packets in receiveFileFromClient: " + totalSize);

            // check that maximum file size
            if (totalSize > getTransferSize())
            {
                sendErrPacket(ERROR.ERR_NOT_DEFINED, "receiveFileFromClient: Exceed MFY buffer size " + totalSize + " > " + getTransferSize());
                tftpLog.logln(USR.ERROR, getClient() + " Client tries to write file and exceed buffer size!");
                return false;
            }
            ...
            catch (IOException e)
            {
                ...
                tftpLog.logln(USR.ERROR, "OutputStream got closed in receiveFileFromClient: " + e.getMessage() + " caused by: " + e.getCause());
                return false;
            }
		...
		{
			tftpLog.logln(USR.ERROR, "problem while closing OutputStream in receiveFileFromClient: " + e.getMessage() + " caused by: " + e.getCause());
			return false;
		}
    ...
    /*		byte[] fileContent,*/
    InputStream is, Address clientAddress, int clientPort, boolean useOptions)
    {
        ...
        tftpLog.logln(USR.ERROR, "clearing socket buffer for " + clientAddress + ":" + clientPort);
        ...
        tftpLog.logln(USR.ERROR, "try sending....");
        ...
        try
        {
            ...
            if (useOptions)
            {
                ...
                tftpLog.logln(USR.ERROR, "ClientHandler: sendFileToClient transfer size = " + getTransferSize());
        ...
        {
            tftpLog.logln(USR.ERROR, "CODING ERRO? this case should be fetched in DataTransfer!");
            return false;
        }
        catch (InstantiationException e)
        {
            tftpLog.logln(USR.ERROR, "CODING ERROR: Could not create DATA and ACK packet in sendFileToClient");
            return false;
        }
        catch (IOException e)
        {
    ...
    void sendDataPacket(int blocknumber, byte[] data, int offset, int dataSize)
    {
        ...
        {
            tftpLog.logln(USR.ERROR, getClient() + " UDP send DATA packet failure.");
            tftpLog.logln(USR.ERROR, e.toString());
        }
        tftpLog.logln(USR.ERROR, "SEND DATA" + " [" + getClient() + "] BN=[" + blocknumber + "]");
    }
    ...
    void sendAckPacket(int blocknumber)
    {
        ...
        {
            tftpLog.logln(USR.ERROR, getClient() + " UDP send ACK packet failure.");
            tftpLog.logln(USR.ERROR, e.toString());
        }
        tftpLog.logln(USR.ERROR, "SEND ACK" + " [" + getClient() + "] BN=[" + blocknumber + "]");
    }
    ...
    void sendErrPacket(int errorCode, String errorMsg)
    {
        ...
        {
            tftpLog.logln(USR.ERROR, getClient() + " UDP send ERROR packet failure.");
            tftpLog.logln(USR.ERROR, e.toString());
            return;
        }
        tftpLog.logln(USR.ERROR, "SEND ERROR" + " [" + getClient() + "] EC = [" + errorCode + "] " + errorMsg);
    }
    ...
    public void setTimeout(int timeout)
    {
        ...
        {
            tftpLog.logln(USR.ERROR, "Could not set socket timeout on worker socket!");
        }
    }
    ...
    public void sendLastAckForDataPacket(DATA data)
    {
        ...
        {
            tftpLog.logln(USR.ERROR, "Could not wait to check if last ack was received by client!");
        }
        finally
        {
