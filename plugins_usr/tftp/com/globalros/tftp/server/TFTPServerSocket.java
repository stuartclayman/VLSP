...
public class TFTPServerSocket implements Runnable {
    ...
    public TFTPServerSocket(int serverPort, int poolSize, VirtualFileSystem vfs, EventListener listener) {
        ...
        tftpLog.logln(USR.STDOUT, "Creating new pool of " + poolSize + " worker threads");
    ...
    public void run() {
        //tftpLog.logln(USR.STDOUT, "TFTPServerSocket: top of run()");
        ...
        try {
            ...
            tftpLog.logln(USR.ERROR, "Could not create socket on port: " + serverPort + ", shutting down!");
            // OK here we should inform the main thread that is is of no use to continue
            stop();
            return;
        }
		
        tftpLog.logln(USR.STDOUT, "TFTPServerSocket is started on port " + serverPort);
        ...
        for (;abort != true;) {
            ...
            clientAddress = packet.getSrcAddress();
            clientPort = packet.getSrcPort();

            tftpLog.logln(USR.STDOUT, "server received request for file from " + clientAddress.toString());

            // copy the data from udp packet      
            byte[] tftpP = new byte[packet.getLength()];
            System.arraycopy(packet.getData(), 0, tftpP, 0, packet.getLength());
            ...
            try {
                ...
                case RRQ.OPCODE: {
                    frq = new RRQ(tftpP); 
                    tftpLog.logln(USR.STDOUT, "RRQ: " + frq.getFileName()  + " " + frq.getTransferSize());
                    break;
                }
                case WRQ.OPCODE: {
                    frq = new WRQ(tftpP); 
                    tftpLog.logln(USR.STDOUT, "WRQ: " + frq.getFileName()   + " " + frq.getTransferSize());
                    break;
                }
                default: {
                    tftpLog.logln(USR.STDOUT, "opcode=" + opcode);
                    continue;
                }
                }
            } catch (InstantiationException e) {
                tftpLog.logln(USR.ERROR, "InstantiationException: " + e.getMessage());
                continue;
            }
        ...
        tftpLog.logln(USR.STDOUT, "TFTPServerSocket is stopped");


        //tftpLog.logln(USR.STDOUT, "TFTPServerSocket: bottom of run()");
