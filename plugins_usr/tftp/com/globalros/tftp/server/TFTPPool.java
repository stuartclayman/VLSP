...
public class TFTPPool
{
    ...
    public TFTPPool(int size, Hashtable connections, VirtualFileSystem vfs, EventListener listener)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            {
                tftpLog.logln(USR.ERROR, birthDate + " is not a valid ObjectName!" + e);
            }
            catch (InstanceAlreadyExistsException e)
            {
                tftpLog.logln(USR.ERROR, "An instance of " + birthDate + " already exists in agent!" + e);
            }
            catch (MBeanRegistrationException e)
            {
                tftpLog.logln(USR.ERROR, birthDate + " could not be registered in agent!" +  e);
            }
            catch (NotCompliantMBeanException e)
            {
                tftpLog.logln(USR.ERROR, birthDate + " does not comply to the Java Management Extensions Intrumentation and Agent Specification, v1.1!" + e);
            }
            /*         catch (MBeanException e)
                     {
                        tftpLog.logln(USR.ERROR, "could not register worker thread as XMBean with JMX agent");
                        e.printStackTrace();
                     } */
            catch (SocketException e)
            {
                tftpLog.logln(USR.ERROR, "Could not create TFTPRequestHandler nr: " + i + " for TFTPPool");
            }
        }
    }
     ...
     */
    synchronized public void performWork(FRQ frq, Address clientAddress, int clientPort)
    {
        ...
        if (existingConnection != null)
        {
            ...
            {
                tftpLog.logln(USR.ERROR, "Hey, I am already working for you. Be patient!");
                return;
            }
        }
        ...
        synchronized (idleWorkers)
        {
            if (idleWorkers.empty())
            {
                ...
                try
                {
                    tftpLog.logln(USR.ERROR, "WARNING: Overload on tftpPool! ReHash!");
                    wt = new TFTPRHThread(birthDate, new TFTPRequestHandler(vfs, listener));
                    wt.start();
                ...
                {
                    tftpLog.logln(USR.ERROR, "ERROR: Can not handle overload!");
                }
            }
            else
            {
    ...
    // implements XMBeanConstants
    {
        ...
        synchronized void wake(FRQ frq, Address clientAddress, int clientPort)
        {
            requestPacket = frq;
        ...
        synchronized public void die()
        {
            ...
            {
                tftpLog.logln(USR.ERROR, on + " is not a valid ObjectName!" + e);
            }
            catch (InstanceNotFoundException e)
            {
                tftpLog.logln(USR.ERROR, "An instance of " + on + " could not be found in agent!" + e);
            }
            catch (MBeanRegistrationException e)
            {
                tftpLog.logln(USR.ERROR, on + " could not be unregistered in agent!" + e);
            }
