...
import plugins_usr.tftp.com.globalros.tftp.common.VirtualFileImpl;
...
public class TFTPRequestHandler
{
    ...
    public TFTPRequestHandler(VirtualFileSystem vfs, EventListener listener) throws SocketException
    {
        ...
        {
            tftpLog.logln(USR.ERROR, "Could not create InitialContext! " + e.toString());
    ...
    public void run(FRQ frq, Address clientAddress, int clientPort)
    {
        if (frq == null)
        {
            tftpLog.logln(USR.ERROR, "TFTPRequestHandler run is called with null packet!");
            return;
        }
        if (clientAddress == null)
        {
            tftpLog.logln(USR.ERROR, "TFTPRequestHandler run is called with invalid client address!");
            return;
        }
        if (clientPort == 0)
        {
            tftpLog.logln(USR.ERROR, "TFTPRequestHandler run is called with invalid client port!");
            return;
        }
        ...
        {
            RRQ rrq = (RRQ) frq;
            tftpLog.logln(USR.ERROR, tftpClient.getClient() + " RRQ " + rrq.getFileName());
            ...
            {
                tftpLog.logln(USR.ERROR, "FileNotFoundException: " + e.getMessage());
            ...
            {
                tftpLog.logln(USR.ERROR, "This is sad but true, we cannot close the inputStream for " + rrq.getFileName());
                return;
            }
        }
        ...
        {
            WRQ wrq = (WRQ) frq;
            tftpLog.logln(USR.ERROR, tftpClient.getClient() + " WRQ " + wrq.getFileName());
            ...
            {
                tftpLog.logln(USR.ERROR, "Exception occurred in TFTPRequestHandler run " + e);
                tftpClient.sendErrPacket(ERROR.ERR_ACCESS_VIOLATION, e.getMessage());
                //e.printStackTrace();
            }
        }
    }
