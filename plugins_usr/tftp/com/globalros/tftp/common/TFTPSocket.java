...
public class TFTPSocket
{
   ...
   private static DatagramSocket getFreeSocket() throws SocketException
   {
      ...
      tftpLog.logln(USR.STDOUT, "Could not find a free port!");
   ...
   public TFTPPacket read() throws IOException
   {
      ...
      byte[] usrData = usrPacket.getData();
      int usrLength = usrPacket.getLength();
      tftpLog.logln(USR.STDOUT, "usrPacket.length in receive: " + usrLength);

      // copy tftpdata
      byte[] tftpPB = new byte[usrPacket.getLength()];
      ...
      try
      {
         ...
         switch (opcode)
         {
            ...
            case ERROR.OPCODE :
               ...
               break;            
            default:
               tftpLog.logln(USR.STDOUT, "Unknown opcode: "+opcode);
               break;   
      ...
      tftpP.setPort(usrPacket.getSrcPort());
      tftpP.setAddress(usrPacket.getSrcAddress());
   ...
   {           
      ...
      tftpLog.logln(USR.STDOUT, "TFTPSocket: write to " + address + ":" + port);
   ...
   public void connect(Address addr, int port)
   {
    ...
    public void clear() 
    {
        try
            ...
            {
                tftpLog.logln(USR.STDOUT, e1.getMessage());
                e1.printStackTrace();
            }
        ...
        while (true)
            {
                try
                    ...
                    {
                        tftpLog.logln(USR.STDOUT, "[clear]: "+ste.getMessage());
                        try
                            {
                    ...
                    {
                        tftpLog.logln(USR.STDOUT, "[clear] :" +e.getMessage());
                        try
                            {
                                usrSocket.setSoTimeout(timeout);
                            } catch (SocketException e1)
        ...
        //            tftpLog.logln(USR.STDOUT, "drop the connection and reconnect...");
        //            
        //            this.connect(tempAddress, tempPort);                                            
        //         }         
        //      } catch(SocketException se)
        //      {
        //         tftpLog.logln(USR.STDOUT, "Socket is freaking useless...:"+se.getMessage()); 
        //      }
