...
Extractor onlyProbeValues = new Extractor()
{
    public Serializable extract(Measurement m){
        ...
        Logger.getLogger("log").logln(USR.APP_EXTRA,
            "Collected: " + object);
        return object;
    }
};
...
enum AggregateFnSpecifer { Average, Variance, SD, Length, Sum, Min,
                           Max };
...
AggregateFn average = new AggregateFn()
{
    public AggregatorMeasurement aggregate(
        ...
        Logger.getLogger("log").logln(
            USR.APP_EXTRA, "Aggregation: average = " + number);
...
AggregateFn variance = new AggregateFn()
{
    public AggregatorMeasurement aggregate(
        ...
        Logger.getLogger("log").logln(
            USR.APP_EXTRA, "Aggregation: variance = " + number);
        // now create a Measurement as the result
...
Second backN = new Second(
    sleepTime.getSeconds(), TimeDirection.BACKWARD);
// From Position.END_OF_INDEX, back 30 seconds
Interval interval = new EndPointInterval(
    (AbsolutePosition)Position.END_OF_INDEX, backN);
...
public ApplicationResponse init(String[] args){
        ...
        if (thisArg.charAt(0) == '-') {
            ...
            case 'a': {
                if (argValue.equals("average")) {
                    ...
                    Logger.getLogger("log").logln(
                        USR.ERROR,
                        "AggPoint: unknown aggregation function "
                        +
                        argValue);
                }
                break;
            ...
            case 'l': {
                ...
                if (potentialPath.isDirectory() &&
                    ...
                    Logger.getLogger("log").logln(
                        USR.ERROR,
                        "AggPoint: cannot write file in directory "
                        + argValue);
                    System.exit(1);
                }
                break;
            ...
            default:
                Logger.getLogger("log").logln(
                    USR.ERROR,
                    "AggPoint: unknown option " + option);
                break;
            }
        }
    }
    ...
    DefaultProbeAttribute pa0 = new DefaultProbeAttribute(
        0, "value", ProbeAttributeType.DOUBLE, "n");
    ...
    if (actualAggregateFn == null)
        return new ApplicationResponse(false,
            "No Aggregation Fn has been set");
...
public ApplicationResponse start(){
    Logger.getLogger("log").logln(
        USR.STDOUT, "AggPoint: input = " + inputDataAddress);
    ...
    Logger.getLogger("log").logln(
        USR.STDOUT, "AggPoint: output = " + outputDataAddress);

    // set up data plane
    // might be null if there is no forwarding required
    DataPlane outputDataPlane = null;
    if (outputDataAddress != null)
        outputDataPlane = new USRDataPlaneProducerWithNames(
            outputDataAddress);
