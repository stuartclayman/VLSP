#!/bin/sh
MASTERSCRIPT=scripts/testbed_time_2hour_noPA.xml
...
OUTPUT=random_testbed_time_long_2hour_noPA
POLICY=Random
rm -f $OUTPUT
count=0
for i in  $SEQ; do
...
POLICY=Pressure
OUTPUT=pressure_testbed_time_long_2hour_noPA
rm -f $OUTPUT
count=0
for i in $SEQ; do
    for j in `seq $REPS`; do
...
POLICY=HotSpot
OUTPUT=hotspot_testbed_time_long_2hour_noPA
rm -f $OUTPUT
count=0
for i in $SEQ; do
    for j in `seq $REPS`; do
